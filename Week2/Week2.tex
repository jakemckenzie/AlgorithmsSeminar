...
\end{enumerate}
\newpage
9. Given two integers $x$ and $n$, say we wrote a function to compute $x^n$ using divide and conquer. What is the recurrence relation and worst case runtime of our function? Please also attempt to write some code that accomplishes this.
\begin{enumerate}
...
\newpage
C. Any comparison based sorting algorithm must make at least f(n) comparisons. Show that is f(n) $\in \Omega(n\log{n})$. Hint: How many possible ways are there are there of choosing, without replacement $N$ things from $N$ where order matters.
\begin{enumerate}
    \item[a)]$\log{n!}$
    \item[b)]$n^2\log{n}$
    \item[c)]$\log{n^n}$

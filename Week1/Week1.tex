...
\end{enumerate}
\newpage
\noindent 4. Using what you know prove or disprove that  if $f(n) \in O(g(n)) $ and $g(n) \in O(h(n))$ then $f(n) \in \Omega(h(n))$.
\newpage
...
\newpage
\noindent 7. Using the technique of splitting the sum, find the result of this summation: $\sum\limits_{k=0}^{\infty}{\frac{k^2}{2^k}}$. Take advantage of the geometric series $\sum\limits_{k=0}^{\infty}{a \cdot r^n} = \frac{a}{1-r}$ for $|r| < 1$.
\newpage
...
\end{wrapfigure}
\noindent 9. Argue given the information I provide which of the following in the list of sorts given are stable or unstable. A sorting algorithm is said to be stable if two objects with equal keys appear in the same order in sorted output as they appear in the input array to be sorted.\\\\
\begin{enumerate}

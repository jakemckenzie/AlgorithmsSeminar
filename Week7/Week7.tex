...
2. Given an arbitary amount of time $t$ what is the maximum amount of spells
we can cast $S$?
\newpage
...
3. Why does the greedy algorithm work in this case?\\\\\\\\\\\\\\\\\\\\
4. Would you say this problem is NP-Complete? NP-Complete problems have the 
...
comes to algorithms.\\\\
5. Of the problems you've covered in class, which does this problem degenerate too? Describe for 
which cases this problems degrenates to that problem you've already covered.\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
6. Can you produce an informal algorithm that attempts to solve this problem?
\newpage
\centerline{\includegraphics[scale = .45]{jogger.jpg}}
\noindent 7. You're given a weighted, undirected graph $G$ with loops(edges that loop back on themselves), 
...
\\\\We've talked previously in this packet about a problem that was disguised as a problem that we already knew. Reduce the jogger problem into the subset sum problem. 
 ...
 other problems in NP reduce to your problem.
\newpage 
\noindent 8. Prove or disprove the following lemma: If each edge weight for a graph is increased by 1, then the minimum spanning tree does not change.\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
\newpage
\noindent Many of us have played physical card games 
and many of those that have have run into the game that stops 
being fun because we've run into a game breaking combinations.
Typically these combinations involve playing cards which call 
other cards which call cards which then call the original card 
, $\dots$, repeated for some large finite amount and or the game
is ended because the combination killed the other player. 
\end{document}